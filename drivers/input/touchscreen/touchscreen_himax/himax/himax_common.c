...
//	  int ret = 0;
	...
	if (!syna_ops->auto_test) {
		seq_puts(s, "Not support auto-test proc node\n");
		return 0;
	}
...
int himax_create_proc(struct touchpanel_data *ts, struct himax_proc_operations *syna_ops)
{
	...
	prEntry_tmp = proc_create_data("ctp_openshort_test", 0664, ts->prEntry_tp, &tp_auto_test_proc_fops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
   ...
   //proc files: /proc/touchpanel/debug_info/himax/register
	...
	prEntry_tmp = proc_create_data("vendor", 0664, prEntry_himax, &himax_proc_vendor_ops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
	}
