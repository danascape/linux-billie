...
#define TPD_DEVICE "himax,hxcommon"
...
int hx83112f_resetgpio_set(struct hw_resource *hw_res, bool on);
...
int hx_dis_rload_0f(void)
{
	...
	do {
		...
		TPD_DETAIL("Now data: tmp[3] = 0x%02X ||tmp[2] =0x%02X || tmp[1] = 0x%02X || tmp[0] = 0x%02X\n",
							tmp_data[3], tmp_data[2], tmp_data[1], tmp_data[0]);
...
void himax_mcu_write_sram_0f(const struct firmware *fw_entry, uint8_t *addr, int start_index, uint32_t write_len)
{
	...
	tmp_addr[1] = addr[1];
	tmp_addr[0] = addr[0];
	TPD_DETAIL("%s, write = 0x%2.2X, = 0x%2.2X, [1] = 0x%2.2X, [0] = 0x%2.2X\n",
					__func__, tmp_addr[3], tmp_addr[2], tmp_addr[1], tmp_addr[0]);
	...
	if (g_chip_info->tmp_data == NULL) {
		...
		g_chip_info->tmp_data = kcalloc(firmware_update_space, sizeof(uint8_t), GFP_KERNEL);
		if (g_chip_info->tmp_data == NULL) {
...
*/
	...
	for (i = 1; i < part_num; i++)
		memcpy(hx83112f_nf_FW_buf + (zf_info_arr[i].cfg_addr - dsram_base),
			(unsigned char *)&fw_entry->data[zf_info_arr[i].fw_addr], zf_info_arr[i].write_size);

	hx83112f_nf_cfg_crc = himax_mcu_Calculate_CRC_with_AP(hx83112f_nf_FW_buf, 0, hx83112f_nf_cfg_sz);
	TPD_INFO("chenyunrui:hx83112f_nf_cfg_crc = %ld\n", hx83112f_nf_cfg_crc);
	kfree(zf_info_arr);
	return true;
}
...
static int hx83112f_nf_zf_part_info(const struct firmware *fw_entry)
{
	...
	TPD_INFO("64K CRC Failed! flag_1k_header= %d\n", flag_1k_header);

	if (flag_1k_header == true)
		crc = himax_sram_write_crc_check(fw_entry, pzf_op->data_sram_start_addr, HX1K, HX64K);
	else
		crc = himax_sram_write_crc_check(fw_entry, pzf_op->data_sram_start_addr, 0, HX64K);
	//crc = himax_hw_check_CRC(pzf_op->data_sram_start_addr, HX64K);
    */
	...
	//do {
		...
		if (crc != hx83112f_nf_cfg_crc) {
			TPD_INFO("Config CRC FAIL, HW CRC = %X, SW CRC = %X, retry time = %d",
							crc, hx83112f_nf_cfg_crc, retry);
			//retry++;
			ret = false;
		} else {
			ret = true;
...
void himax_mcu_firmware_update_0f(const struct firmware *fw_entry)
{
	...
	if (private_ts->suspend_state != TP_RESUME_COMPLETE) {
...
int hx_0f_init(void)
{
	...
	himax_in_parse_assign_cmd(zf_addr_dis_flash_reload, pzf_op->addr_dis_flash_reload,
								sizeof(pzf_op->addr_dis_flash_reload));
	himax_in_parse_assign_cmd(zf_data_dis_flash_reload, pzf_op->data_dis_flash_reload,
...
void himax_read_OP_FW_ver(struct chip_data_hx83112f *chip_info)
{
	uint8_t cmd[4];
	uint8_t data[4];
...
int himax_report_data_init(int max_touch_point, int tx_num, int rx_num)
{
	...
	if (hx_touch_data->raw_cnt_rmd != 0x00) {//more than 4 fingers
		hx_touch_data->rawdata_size = cal_data_len
				(hx_touch_data->raw_cnt_rmd, max_touch_point, hx_touch_data->raw_cnt_max);
		hx_touch_data->touch_info_size = (max_touch_point + hx_touch_data->raw_cnt_max + 2) * 4;
	} else {//less than 4 fingers
		hx_touch_data->rawdata_size =
				cal_data_len(hx_touch_data->raw_cnt_rmd, max_touch_point, hx_touch_data->raw_cnt_max);
	...
	if ((tx_num * rx_num + tx_num + rx_num) % hx_touch_data->rawdata_size == 0)
		...
		hx_touch_data->rawdata_frame_size =
				(tx_num * rx_num + tx_num + rx_num) / hx_touch_data->rawdata_size + 1;

	TPD_INFO("%s: rawdata_frame_size = %d ", __func__, hx_touch_data->rawdata_frame_size);
	TPD_INFO("%s: max point:%d, max:%d,  raw_cnt_rmd:%d, rawdata_size:%d, touch_info_size:%d\n",
				__func__, max_touch_point, hx_touch_data->raw_cnt_max, hx_touch_data->raw_cnt_rmd,
...
void himax_log_touch_data(uint8_t *buf, struct himax_report_data *hx_touch_data)
{
	...
	for (loop_i = 0; loop_i < print_size; loop_i++) {
		//if (loop_i % 8 == 0)
			//TPD_INFO("KERN_CONT [himax]");

		TPD_DETAIL(KERN_CONT "0x%02X ",  buf[loop_i]);
		if ((loop_i + 1) % 8 == 0)
...
void himax_idle_mode(int disable)
{
	...
	do {
		...
		himax_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO("%s:After turnON/OFF IdleMode [0]=0x%02X,[1]=0x%02X,[2]=0x%02X,[3]=0x%02X\n",
					__func__, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
						 ...
						 char *g_project_test_info_log, char *rslt_buf, char *filepath)
{
	...
	"Project_name: %s%d\nFW_ID: %8X\nFW_Ver: %4X\nPanel Info: TX_Num=%d RX_Num=%d\nTest stage: Mobile\n",
		project_name_log, 86260,
			chip_info->fw_id, chip_info->fw_ver, chip_info->hw_res->TX_NUM, chip_info->hw_res->RX_NUM);
	//snprintf(g_project_test_info_log, 118,
	//"Project_name: %s%d\nFW_ID: %8X\nFW_Ver: %4X\nPanel Info: TX_Num=%d RX_Num=%d\nTest stage: Mobile\n",
	//project_name_log, 12345, chip_info->fw_id, chip_info->fw_ver, chip_info->hw_res->TX_NUM,
	//chip_info->hw_res->RX_NUM);//get_project() = 12345
	/*Himax_DB_Test End*/
...
int himax_get_rawdata(struct chip_data_hx83112f *chip_info, uint32_t *RAW, uint32_t datalen)
{
	...
	for (j = 0; j < chip_info->hw_res->TX_NUM; j++) {
		if (j == 0)
			TPD_DETAIL(KERN_CONT "[himax]	   RX%2d", j + 1);
		else
			TPD_DETAIL(KERN_CONT "	RX%2d", j + 1);

	}
	TPD_DETAIL(KERN_CONT "\n");

	for (i = 0; i < chip_info->hw_res->RX_NUM; i++) {
		TPD_DETAIL(KERN_CONT "[himax]TX%2d", i + 1);
		for (j = 0; j < chip_info->hw_res->TX_NUM; j++) {
			...
			TPD_DETAIL(KERN_CONT "%6d", RAW[index]);
...
uint32_t himax_wait_sorting_mode(uint8_t checktype)
{
	...
	do {
		...
		tmp_addr[0] = 0x04;
		himax_register_read(tmp_addr, 4, tmp_data, false);
		TPD_INFO("%s: tmp_data[0]=%x, tmp_data[1]=%x\n", __func__, tmp_data[0], tmp_data[1]);
	...
	case HIMAX_INSPECTION_LPWUG_NOISE:
		...
		if (isRead_csv == false) {
			...
			if (weight > LPWUG_NOISEMAX ||
					weight < hx83112f_nf_inspection_criteria[IDX_LPWUG_NOISE_MIN][i]) {
				TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_NOISE FAIL\n", __func__);
				ret = THP_AFE_INSPECT_ENOISE;
			}
		}
...
/* get idx of criteria where parsing file */
	...
	for (i = 0; i < HX_CRITERIA_SIZE; i++) {
		...
		if (hx83112f_nf_inspt_crtra_flag[i] == 1) {
			for (j = 0; j < all_mut_len; j++) {
				TPD_DETAIL(KERN_CONT "%d, ", hx83112f_nf_inspection_criteria[i][j]);
				if (j % 16 == 15)
					TPD_DETAIL(KERN_CONT "\n");
			}
		} else {
			TPD_DETAIL(KERN_CONT "No this Item in this criteria file!\n");
		}
		TPD_DETAIL(KERN_CONT "\n");
	}
...
static void himax_limit_get(struct touchpanel_data *ts, struct hx_limit_data *limit)
{
	...
	if (hx83112f_nf_inspt_crtra_flag == NULL || hx83112f_nf_inspection_criteria == NULL) {
		TPD_INFO("%s: %d, Memory allocation failed!\n", __func__, __LINE__);
		goto FAIL_ALLOC_MEM;
	}
	...
	for (i = 0; i < HX_CRITERIA_SIZE; i++) {
		if (!hx83112f_nf_inspection_criteria[i]) {
			hx83112f_nf_inspection_criteria[i] = kcalloc(
					(g_chip_info->hw_res->TX_NUM * g_chip_info->hw_res->RX_NUM),
	...
	limit->item_name = kcalloc(limit->item_size, sizeof(char *), GFP_KERNEL);
	for (i = 0; i < limit->item_size; i++) {
		if (g_hx_inspt_crtra_name[i] != NULL) {
			limit->item_name[i] = kcalloc(strlen(g_hx_inspt_crtra_name[i]), sizeof(char), GFP_KERNEL);
			memcpy(limit->item_name[i], g_hx_inspt_crtra_name[i],
					sizeof(char) * strlen(g_hx_inspt_crtra_name[i]));
		}
	}
	limit->crtra_val = kzalloc(sizeof(int *) * limit->item_size, GFP_KERNEL);
	for (i = 0; i < limit->item_size; i++)
		limit->crtra_val[i] = kcalloc(limit->rawdata_size, sizeof(int), GFP_KERNEL);
	...
	if (result == NULL) {
		...
		if (result != NULL) {
			for (i = 0 ; i < data_size; i++) {
				...
				if (result[i] == NULL) {
					TPD_INFO("%s: rst_arr Memory allocation failed!\n", __func__);
					goto rst_arr_mem_alloc_failed;
				}
			}
		} else {
			TPD_INFO("%s: Memory allocation failed!\n", __func__);
...
static int himax_parse_criteria_file(struct chip_data_hx83112f *chip_info)
{
	...
	if (result == NULL) {
		...
		if (result != NULL) {
			...
			mdelay(3);
			TPD_INFO("%s: result is NULL, alloc memory the second time.\n", __func__);
			result = kcalloc(data_size, sizeof(char *), GFP_KERNEL);
			if (result != NULL) {
				for (i = 0 ; i < data_size; i++) {
					result[i] = kcalloc(str_max_len, sizeof(char), GFP_KERNEL);
		...
		TPD_INFO("%s: result is not the first, no need alloc memory.\n", __func__);
		for (i = 0 ; i < data_size; i++)
			memset(result[i], 0x00, str_max_len * sizeof(char));
	}
...
static int himax_self_test_data_init(struct chip_data_hx83112f *chip_info)
{
	...
	/* init criteria data*/
	hx83112f_nf_inspt_crtra_flag = kcalloc(HX_CRITERIA_SIZE, sizeof(int), GFP_KERNEL);
	hx83112f_nf_inspection_criteria = kzalloc(sizeof(int *)*HX_CRITERIA_SIZE, GFP_KERNEL);
	if (hx83112f_nf_inspt_crtra_flag == NULL || hx83112f_nf_inspection_criteria == NULL) {
		TPD_INFO("%s: %d, Memory allocation failed!\n", __func__, __LINE__);
		return MEM_ALLOC_FAIL;
...
static void hx83112f_black_screen_test(void *chip_data, char *message)
{
	...
	hx83112f_nf_fail_write_count += snprintf(g_Test_list_log + hx83112f_nf_fail_write_count,
							32, "9. MP_LPWUG_IDLE_TEST_NOISE: %s\n", error ? "NG" : "Ok");
	TPD_INFO("%s", buf);
	snprintf(message, sizeof(message), "%s\n", buf);
	if (error != 0)
		error_num++;
	...
	snprintf(g_file_name_NG, sizeof(g_file_name_NG), "tp_testlimit_gesture_NG_%02d%02d%02d-%02d%02d%02d-utc.csv",
			(rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
			rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	...
	if (error) {
		...
		hx_test_data_pop_out(chip_info, g_Test_list_log, g_Company_info_log,
							g_project_test_info_log, g_rslt_data, g_file_path_NG);
	} else {
		...
		hx_test_data_pop_out(chip_info, g_Test_list_log, g_Company_info_log,
							g_project_test_info_log, g_rslt_data, g_file_path_OK);
	}
	/*Save Log Data */


	snprintf(message, sizeof(message), "%d errors. %s", error_num, error_num ? "" : "result=1");
	TPD_INFO("%d errors. %s\n", error_num, error_num ? "" : "result=1");
 ...
 */
	...
	do {
		error = mpTestFunc(chip_info, HIMAX_INSPECTION_NOISE, (chip_info->hw_res->TX_NUM *
			chip_info->hw_res->RX_NUM) + chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM);
		retry_cnt--;
	} while ((error == RESULT_RETRY) && (retry_cnt > 0));
	...
	do {
		...
		retry_cnt++;
	} while ((tmp_data[3] != back_data[3] || tmp_data[2] != back_data[2] || tmp_data[1] !=
			back_data[1] || tmp_data[0] != back_data[0]) && retry_cnt < HIMAX_REG_RETRY_TIMES);
...
int himax_write_read_reg(uint8_t *tmp_addr, uint8_t *tmp_data, uint8_t hb, uint8_t lb)
{
	...
	if (cnt >= 99) {
		TPD_INFO("%s ERR Now register 0x%08X : high byte = 0x%02X, low byte = 0x%02X\n",
...
void himax_get_DSRAM_data(uint8_t *info_data, uint8_t x_num, uint8_t y_num)
{
	...
	for (i = 2; i < total_size; i = i + 2) { /* 2:PASSWORD NOT included */
		...
		TPD_DETAIL(KERN_CONT "0x%2x:0x%4x ", temp_info_data[i], check_sum_cal);
	}
...
void himax_ts_diag_func(struct chip_data_hx83112f *chip_info, int32_t *mutual_data)
{
	...
	write_buf = kzalloc((total_size * 3)*sizeof(char), GFP_KERNEL);
	if (write_buf == NULL) {
		kfree(info_data);
		info_data = NULL;
		return;
	...
	if (hx_touch_data->hx_rawdata_buf[0] == 0x3A
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) {
				...
				hx_touch_data->hx_rawdata_buf[i * 2 + 4]; /* 4: RawData Header, 1:HSB  */
			} else {
				//self
				temp1 = i + index;
...
bool diag_check_sum(struct himax_report_data *hx_touch_data) /*return checksum value  */
{
	...
	for (i = 0, check_sum_cal = 0;
		i < (hx_touch_data->touch_all_size - hx_touch_data->touch_info_size); i = i + 2)
...
static int hx83112f_get_touch_points(void *chip_data, struct point_info *points, int max_num)
{
	...
	if (g_diag_command) {
		...
		if (!diag_check_sum(hx_touch_data))
			goto err_workqueue_out;
		diag_parse_raw_data(hx_touch_data, mutual_num, self_num,
				g_diag_command, hx_touch_data->diag_mutual, diag_self);
	}
...
static uint32_t hx83112f_get_fw_id(struct chip_data_hx83112f *chip_info)
{
	...
	TPD_INFO("%s : [0] = 0x%2.2X, [1] = 0x%2.2X, [2] = 0x%2.2X, [3] = 0x%2.2X\n",
				__func__, data[0], data[1], data[2], data[3]);
		...
		struct resolution_info *resolution_info, struct panel_info *panel_data)
{
	...
	if (panel_data->manufacture_info.version) {
		//sprintf(panel_data->manufacture_info.version, "0x%x-%d",
					//panel_data->TP_FW, g_lcd_vendor);
...
void himax_ultra_enter(void)
{
	...
	do {
		...
		TPD_INFO("%s:retry %d, addr = 0x34, correct = 0x%2.2X\n",
	...
	do {
		...
		TPD_INFO("%s:retry %d, addr = 0x33, correct = 0x%2.2X\n",
					__func__, rtimes, tmp_data[0]);
		rtimes++;
	...
	do {
		...
		TPD_INFO("%s:retry %d, addr = 0x33, 0xAA = 0x%2.2X\n",
					__func__, rtimes, tmp_data[0]);
		rtimes++;
...
static int hx83112f_enable_black_gesture(struct chip_data_hx83112f *chip_info, bool enable)
{
	...
	uint8_t tmp_data[4] = {0};
	uint8_t back_data[4] = {0};
	static char psensor_delay_done = 0;
	...
	if (ts->is_suspended) {
		if (enable) {
			...
			if (psensor_delay_done == 0) {
				TPD_INFO("%s, first enter ultra \n", __func__);
				msleep(100);
			}
			p_sensor_rec = true;
			psensor_delay_done = 1;
			himax_ultra_enter();
		}
	} else {
		g_zero_event_count = 0;
		...
		himax_flash_write_burst(tmp_addr, tmp_data);
		//himax_sense_on(0);
		psensor_delay_done = 0;
		TPD_INFO("%s, psensor_delay_done=%d\n", __func__,psensor_delay_done);
	}
	...
	if (!on) {//jitter off
		do {
			if (rtimes > 10) {
				...
				TPD_INFO("%s:0x5A, 0xA5, 0x5A, 0xA5\n", __func__);
				ret = -1;
				break;
			}
			...
			TPD_INFO("%s:retry %d, [0, 1, 2, 3] = 0x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
		...
		do {
			if (rtimes > 10) {
				...
				TPD_INFO("%s:correct tmp_data 0x00, 0x00, 0x00, 0x00\n", __func__);
				ret = -1;
				break;
			}
			...
			TPD_INFO("%s:retry %d, current = 0x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
					__func__, rtimes, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
			rtimes++;
		} while (tmp_data[3] == 0xA5 && tmp_data[2] == 0x5A
				 && tmp_data[1] == 0xA5 && tmp_data[0] == 0x5A);
		...
		if (enable) {/* insert headset */
			do {
				if (rtimes > 10) {
					...
					TPD_INFO("%s:correct data 0x5A, 0xA5, 0x5A, 0xA5\n", __func__);
					ret = -1;
					break;
				}
				...
				himax_register_read(tmp_addr, 4, tmp_data, false);
				TPD_DETAIL("%s:retry %d, current [0, 1, 2, 3] = 0x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
					__func__, rtimes, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
				rtimes++;
			} while (tmp_data[3] != 0xA5 || tmp_data[2] != 0x5A
					 || tmp_data[1] != 0xA5 || tmp_data[0] != 0x5A);
			...
			do {
				if (rtimes > 10) {
					...
					TPD_INFO("%s:correct 0x5A, 0xA5, 0x5A, 0xA5\n", __func__);
					ret = -1;
					break;
				}
...
static int hx83112f_rotative_switch(struct chip_data_hx83112f *chip_info, int mode)
{
	...
	TPD_INFO("fw_edge = %d, mode = %d.ts->limit_switch = %d\n", ts->fw_edge_limit_support, mode, ts->limit_switch);
	if (ts->fw_edge_limit_support) {
		if (ts->limit_switch == 0 && VERTICAL_SCREEN == chip_info->touch_direction) {/* vertical */
			do {
				if (rtimes > 10) {
					TPD_INFO("%s:rotative normal failed!\n", __func__);
					TPD_INFO("%s: [0, 1, 2, 3] = 0x5A, 0xA5, 0x5A, 0xA5\n",
								__func__);
					ret = -1;
					break;
				}
				...
				himax_register_read(tmp_addr, 4, tmp_data, false);
				TPD_INFO("%s:retry %d, [0, 1, 2, 3] = x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
						__func__, rtimes, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
				rtimes++;
			} while (tmp_data[3] != 0xA5 || tmp_data[2] != 0x5A
					|| tmp_data[1] != 0xA5 || tmp_data[0] != 0x5A);
			...
			rtimes = 0;
			if (ts->limit_switch == 3 || chip_info->touch_direction == LANDSCAPE_SCREEN_270) { //turn right
				do {
					if (rtimes > 10) {
						TPD_INFO("%s:rotative right failed!\n", __func__);
						TPD_INFO("%s:[0, 1, 2, 3] = 0x3A, 0xA3, 0x3A, 0xA3\n", __func__);
						ret = -1;
						break;
					}
			...
			} else if (ts->limit_switch == 1) { //turn left
				do {
					if (rtimes > 10) {
						TPD_INFO("%s:rotative left failed!\n", __func__);
						TPD_INFO("%s:correct [0, 1, 2, 3] = x1A, 0xA1, 0x1A, 0xA1\n",
									__func__);
						ret = -1;
						break;
					}
		...
		if (mode) {//open
			do {
				if (rtimes > 10) {
					...
					TPD_INFO("%s:correct [0, 1, 2, 3] = 0x5A, 0xA5, 0x5A, 0xA5\n",
								__func__);
					ret = -1;
					break;
				}
				...
				himax_register_read(tmp_addr, 4, tmp_data, false);
				TPD_INFO("%s:retry %d, current[0, 1, 2, 3] = 0x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
						__func__, rtimes, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
				rtimes++;
			} while (tmp_data[3] != 0xA5 || tmp_data[2] != 0x5A
					 || tmp_data[1] != 0xA5 || tmp_data[0] != 0x5A);
			...
			do {
				...
				himax_register_read(tmp_addr, 4, tmp_data, false);
				TPD_INFO("%s:retry %d,tmp[0, 1, 2, 3] = 0x%2.2X, 0x%2.2X, 0x%2.2X, 0x%2.2X\n",
					__func__, rtimes, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
				rtimes++;
...
static int hx83112f_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	for (i = 0; i < 128; i++) {
		if (!i)
			TPD_DETAIL(KERN_CONT "%s: gesture buf data\n", __func__);


		if (i % 8 == 0)
			TPD_DETAIL(KERN_CONT "[himax]");

		TPD_DETAIL(KERN_CONT "%02d ", buf[i]);
	...
	if (buf[GEST_PTLG_ID_LEN] == GEST_PTLG_HDR_ID1 &&
		...
		if (check_point_format == 0) {
			...
			while (i < (gest_len + 1) / 2) {
				...
				gest_pt_y[gest_pt_cnt] = buf[GEST_PTLG_ID_LEN + 4 + i * 2 + 1]
...
static int hx83112f_int_pin_test(struct seq_file *s, void *chip_data,
					struct syna_testdata *syna_testdata, char *g_Test_list_log)
{
	...
	if (eint_count == 10) {
		...
		hx83112f_nf_fail_write_count +=
			snprintf(g_Test_list_log + hx83112f_nf_fail_write_count,
				45, "eint_status is low, TP EINT direct stort\n");
...
static void hx83112f_auto_test(struct seq_file *s, void *chip_data, struct syna_testdata *syna_testdata)
{
	...
	p_node = strnstr(private_ts->panel_data.fw_name, ".", MAX_FW_NAME_LENGTH);
	...
	himax_sense_on(0x00);
	himax_check_remapping();
	himax_read_OP_FW_ver(chip_info);
	...
	snprintf(g_file_name_OK, sizeof(g_file_name_OK),  "tp_testlimit_OK_%02d%02d%02d-%02d%02d%02d-utc.csv",
			(rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
			rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	snprintf(g_file_name_NG, sizeof(g_file_name_NG),  "tp_testlimit_NG_%02d%02d%02d-%02d%02d%02d-utc.csv",
			(rtc_now_time.tm_year + 1900) % 100, rtc_now_time.tm_mon + 1, rtc_now_time.tm_mday,
			rtc_now_time.tm_hour, rtc_now_time.tm_min, rtc_now_time.tm_sec);
	...
	seq_printf(s, "%s\n", error_count ? "result=0" : "result=1");
	TPD_INFO(" TP auto test %d error(s). %s\n", error_count, error_count ? "" : "result=1");
...
static void hx83112f_read_debug_data(struct seq_file *s, void *chip_data, int debug_data_type)
{
	...
	case DEBUG_DATA_DELTA:
		seq_puts(s, "Delta data:\n");
		TPD_INFO("Delta data:\n");
		break;

	case DEBUG_DATA_DOWN:
		seq_puts(s, "Finger down data:\n");
		TPD_INFO("Finger down data:\n");
		break;

	default:
		seq_puts(s, "No this debug datatype\n");
...
static int himax_enable_single_tap(void *chip_data, bool enable)
{
	struct chip_data_hx83112f *chip_info = (struct chip_data_hx83112f *)chip_data;
...
static struct touchpanel_operations hx83112f_ops = {
	...
	.enable_single_tap		= himax_enable_single_tap,
	/*Himax_DB_Test Start*/
...
static void himax_init_op_apk_op(struct touchpanel_data *ts)
{
...
static int hx83112f_tp_probe(struct spi_device *spi)
{
	...
	chip_info->himax_0f_update_wq = create_singlethread_workqueue("HMX_0f_update_request");
...
static void hx83112f_tp_shutdown(struct spi_device *spi)
{
	int rc;
	int i = 100;
	struct touchpanel_data *ts = spi_get_drvdata(spi);
	...
	*/
	...
	mdelay(5);
	gpio_direction_output(ts->hw_res.cs_gpio, 0);
	while(gpio_get_value(ts->hw_res.cs_gpio) != 0&&(i>0)){
